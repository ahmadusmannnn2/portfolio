<x-app-layout>
                    ...
                    <form action="{{ route('admin.portfolios.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <label
                                for="image"
                                id="image-label"
                                class="inline-flex items-center px-4 py-2 bg-theme-secondary text-theme-text rounded-lg shadow-sm tracking-wide uppercase border border-transparent cursor-pointer hover:bg-theme-main hover:text-white mt-1"
                            >
    ...
    {{-- Skrip untuk memperbarui teks label saat file dipilih --}}
    @push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            inputImage.addEventListener('change', function () {
                if (inputImage.files.length > 0) {
                    labelText.textContent = inputImage.files[0].name;
                } else {
